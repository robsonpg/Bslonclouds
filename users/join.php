    ...
    if ($validation->passed()) {
        ...
        try {
            ...
            if ($act == 1 || !$allowPasswords) {
                ...
                logger(null, 'Enviando email', $to . ' ' . $subject . ' ' . $body);
            }
        } catch (Exception $e) {
            ...
            logger(null, 'Exception Caught', Input::sanitize($e->getMessage()));
            exit('A system error occurred. We apologize for the inconvenience. Logging is available.');
        }
        if ($form_valid == true) {
...
<script nonce="<?php echo htmlspecialchars($userspice_nonce ?? ''); ?>" type="text/javascript">
